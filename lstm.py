...
def get_labels():
    ...
    labels_df = db_connector.run_query('SELECT symbol, date, close'
                                       ...
                                       "WHERE symbol in ('SPHD', 'VT', 'VYM', 'VTI')")
...
def define_and_train_model(X_train, y_train, X_test, y_test):
    """
    ...
    model.add(LSTM(X_train.shape[2] // 4, activation='relu', input_shape=(X_train.shape[1], X_train.shape[2]),
                   return_sequences=True))
