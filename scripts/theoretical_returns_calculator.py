...
class OptionPricingModel:
    def __init__(self, symbol: str, date: str, dividend_yield: float = .02):
        """
    ...
    def black_scholes_american(self, S: float, K: float, T: float, option_type: str = 'call') -> float:
        """
        ...
        if option_type == 'call':
            option_price = (
                        S * np.exp(-q * T) * si.norm.cdf(d1, 0.0, 1.0) - K * np.exp(-r * T) * si.norm.cdf(d2, 0.0, 1.0))
        else:
            option_price = (K * np.exp(-r * T) * si.norm.cdf(-d2, 0.0, 1.0) - S * np.exp(-q * T) * si.norm.cdf(
                -d1, 0.0, 1.0
                ))
        return option_price
...
class OptionsReturnCalculator:
    ...
    def calculate_option_returns(self):
        ...
        for index, row in self.backtest_results_dataframe.iterrows():
            try:
                ...
                net_return, final_price = self._execute_trade(row, strike, option_type, num_contracts, premium_price)
                self.account_balance += net_return

                # Store the trade details in a dictionary and add it to the list
                trade_details = {
                    'Trade Date': trade_date, 'Stock Price': current_stock_price, 'Strike Price': strike,
                    'Time to Expiry': time_to_expiry, 'Option Type': option_type, 'Number of Contracts': num_contracts,
                    'Cost of Buying': cost_of_buying, 'Premium Price': premium_price, 'Net Return': net_return,
    ...
    def _calculate_num_contracts_and_cost(
        ...
        premium_price = model.black_scholes_american(S=current_stock_price, K=strike, T=time_to_expiry, option_type=option_type)
...
calculator = OptionsReturnCalculator(
    symbol, df, initial_investment=124000, max_investment_percent=.2, option_position='OTM'
    )
