    ...
    def _get_labels(self) -> pd.DataFrame:
        ...
        try:
            labels_df = db_connector.run_query(
                f"SELECT date, close FROM {self.etf_symbol}.{self.etf_symbol}_historical_prices WHERE symbol='{self.etf_symbol}'"
            )
...
class ETFPredictor:
    ...
    def _build_model(self):
        ...
        model.add(
            LSTM(
                units=20, return_sequences=True, input_shape=(self.sequence_length, self.train_data.shape[1] - 2)
            )
        )
    ...
    def _sequence_predict(self, X_next):
        """
        Predict the next sequence based on the given sequence X_next.
        """
        last_sequence = X_next[-self.sequence_length:]
        last_sequence = last_sequence.reshape((1, self.sequence_length, self.train_data.shape[1] - 2))
        predicted_sequence = self.model.predict(last_sequence)[0]
        return predicted_sequence


    def backtest(self, window_length=100, overlap=50):
        """
        ...
        step_size = window_length - overlap  # This determines the number of new days introduced in each window.
        ...
        for i in range(n_windows):
            ...
            next_data = self.train_data.iloc[end_idx: end_idx + self.sequence_length]
            print(train_window)
            print(next_data)
            ...
            # Extract the feature matrix from next_data and predict the next sequence
            X_next = self.scaler.transform(next_data.drop(columns=["date", "close"]).values)
            predicted_sequence = self._sequence_predict(X_next)
            print(predicted_sequence)
            # Measure the prediction error
            true_sequence = next_data["close"].values[-self.sequence_length:]
            print(true_sequence)
            error = np.mean(np.abs(true_sequence - predicted_sequence))
            errors.append(error)
            self.model = self._build_model()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--days_ahead", type=int, default=7, help="How many days ahead are we forecasting?")
    ...
    self = DatasetBuilder(table_names=tables, etf_symbol=etf_arg, forecast_n_days_ahead=days_ahead)

    train_data, predict_data = self.build_datasets()
    train_data.drop(columns="date_label", inplace=True)

    if args.backtest:
        predictor = ETFPredictor(train_data=train_data,
        ...
        mae = predictor.backtest(window_length=100, overlap=50)
