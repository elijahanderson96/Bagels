    ...
    for file_name in glob.glob(os.path.join(etf_results_dir, "*.json")):
        with open(file_name, "r") as file:
            ...
            if data.get(target_param, None) == target_value:
                metric = (
                    data["results"]["mae"] if "mae" in data["results"] else float("inf")
                )
                config = data
...
def main():
    parser = argparse.ArgumentParser(
        description="Find the best configuration for a specific ETF"
    )
    parser.add_argument(
        "--etf_name", type=str, required=True, help="ETF name to search results for"
    )
    parser.add_argument(
        "--target_param",
        type=str,
        required=True,
        help="Target parameter to filter results",
    )
    parser.add_argument(
        "--target_value", type=int, required=True, help="Value of the target parameter"
    )
    ...
    best_config, best_metric = find_best_configuration(
        results_dir, etf_name, target_param, target_value
    )
