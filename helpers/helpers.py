...
def parse_sql_file(full_path, delimiter='-- @', name_delim='~'):
    ...
    for tmp_i, tmp_c in enumerate(parsed_sql):
        ...
        delim_index = [tmp_c_split.index(x) for x in tmp_c_split if re.search(pattern=name_delim, string=x) is not None]

        tmp_name = re.sub(pattern=name_delim, repl="", string=tmp_c_split[delim_index[0]])
        tmp_name = re.sub(pattern=r'\s+', repl="_", string=tmp_name)

        ind_to_remove_comm = [tmp_c_split.index(x) for x in tmp_c_split if
                              re.search(pattern='--', string=x) is not None]
        ind_to_remove_name = [tmp_c_split.index(x) for x in tmp_c_split if
                              re.search(pattern=name_delim, string=x) is not None]
