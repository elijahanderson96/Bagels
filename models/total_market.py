...
class Model:
    def __init__(self, model_type='total'):
    ...
    def one_hot_encode(self):
        ...
        self.prediction_dataset = self.prediction_dataset.join(
            self.close.head(len(self.prediction_dataset)))  # super jank
    ...
    def batch(self):
        ...
        self.prediction_dataset_batched = tf.data.Dataset.from_tensor_slices(
            (self.normalized_prediction_dataset[:, :-1],
             self.normalized_prediction_dataset[:, -1]))
        self.prediction_dataset_batched = self.prediction_dataset_batched.batch(1, drop_remainder=True).batch(1,
                                                                                                              drop_remainder=True)
    ...
    def renormalize(self):
        x = list(
            self.training_preds.as_numpy_iterator())  # do not renormalize any training data thats not in batches of 1
